{{-- filepath: resources/views/categories/edit.blade.php --}}
<x-app-layout>
    <x-slot name="header">
                ...
                <form action="{{ route('categories.update', $categories->id) }}" method="POST">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="description" id="description" rows="4"
                            class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-300">{{ old('description', $categories->description) }}</textarea>
                        @error('description')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                        @enderror
                    </div>
